...
const emit = defineEmits(['admin-logged-in', 'login-failed']);
const { t } = useI18n();
...
<template>
  <form class="iam-form" @submit.prevent="submitLogin">
    <h2>{{ t('identity-and-access-management.admin-login.title') }}</h2>

    <div class="field mb-3">
      <label for="admin-login-email">{{ t('identity-and-access-management.fields.email') }}</label>
      <pv-input-text id="admin-login-email" v-model="email" type="email" class="w-full" required/>
    </div>

    <div class="field mb-3">
      <label for="admin-login-password">{{ t('identity-and-access-management.fields.password') }}</label>
      <pv-input-text id="admin-login-password" v-model="password" type="password" class="w-full" required/>
    </div>
